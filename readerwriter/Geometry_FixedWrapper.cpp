...
// Add a new line in read() to avoid dead-lock for some old .osgb versions...
// Don't know why it happens. Why check() failed to find the problem?
...
namespace osgVerse
{
    typedef osg::Geometry MyClass;
    static osg::Object* wrapper_createinstancefunc_GeometryFixed() { return new osg::Geometry; }
    extern void wrapper_propfunc_GeometryFixed(osgDB::ObjectWrapper*);

    void replaceWrapper_GeometryFixed()
    {
        std::string associates = "osg::Object osg::Node osg::Drawable osg::Geometry";
        osg::ref_ptr<osgDB::ObjectWrapper> wrapper = new osgDB::ObjectWrapper(
            wrapper_createinstancefunc_GeometryFixed, "osg::Geometry", associates);
        wrapper_propfunc_GeometryFixed(wrapper.get());

        osgDB::Registry* registry = osgDB::Registry::instance();
        registry->getObjectWrapperManager()->addWrapper(wrapper.get());
    }

    void wrapper_propfunc_GeometryFixed(osgDB::ObjectWrapper* wrapper)
    {
        OSG_NOTICE << "[osgVerse] Fixed wrapper of osg::Geometry loaded" << std::endl;
        {
    ...
    OSGVERSE_RW_EXPORT bool fixOsgBinaryWrappers(const std::string& libName)
    {
        ...
        {
            if (itr->first == "osg::Geometry")
            { registry->getObjectWrapperManager()->removeWrapper(itr->second.get()); break; }
        }
        replaceWrapper_GeometryFixed(); return true;
    }
}
