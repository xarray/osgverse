...
public:
    ...
    const Layer* getLayer(const std::string_view& name) const noexcept {
        //auto hit = std::ranges::find_if(layers, [&](const auto& layer) { return layer.getName() == name; });  // WangRui 202602: downgrade to C++17
        auto hit = std::find_if(layers.begin(), layers.end(), [&](const auto& layer) { return layer.getName() == name; });  ////
