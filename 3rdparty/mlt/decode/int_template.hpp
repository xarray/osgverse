...
/*template <typename T, typename TTarget>
    requires((std::is_integral_v<T> || std::is_enum_v<T>) && (std::is_integral_v<TTarget> || std::is_enum_v<TTarget>) &&
             sizeof(T) <= sizeof(TTarget))*/  // WangRui 202602: downgrade to C++17
template <typename T, typename TTarget>
////
void IntegerDecoder::decodeIntArray(const T* values,
    ...
    switch (streamMetadata.getLogicalLevelTechnique1()) {
        case LogicalLevelTechnique::NONE: {
            ...
            /*const auto f = isSigned ? [](T x) noexcept { return static_cast<TTarget>(decodeZigZag(x)); }
                                      : [](T x) noexcept { return static_cast<TTarget>(x); };
            std::transform(values, values + count, out, f);*/  // WangRui 202602: downgrade to C++17
            if (isSigned)
                std::transform(values, values + count, out, [](T x) noexcept { return static_cast<TTarget>(decodeZigZag(x)); });
            else
                std::transform(values, values + count, out, [](T x) noexcept { return static_cast<TTarget>(x); });
            ////
            return;
        }
        case LogicalLevelTechnique::DELTA:
        ...
        case LogicalLevelTechnique::RLE: {
            ...
            */  // WangRui 202602: downgrade to C++17
            if (isSigned)
            {
                auto decoder = [](T x) { return static_cast<TTarget>(decodeZigZag(x)); };
                rle::decodeInt<T, TTarget>(values, count, out, outCount, rleMetadata.getRuns(), std::move(decoder));
            }
            else
            {
                auto decoder = [](T x) { return static_cast<TTarget>(x); };
                rle::decodeInt<T, TTarget>(values, count, out, outCount, rleMetadata.getRuns(), std::move(decoder));
            }
            ////
            break;
...
/*template <typename TDecode, typename TTarget>
    requires(std::is_integral_v<TDecode> && (std::is_integral_v<TTarget> || std::is_enum_v<TTarget>))*/  // WangRui 202602: downgrade to C++17
template <typename TDecode, typename TTarget>
////
...
void IntegerDecoder::decodeZigZagDelta(const T* values,
    ...
    ST previousValue = 0;
    for (const auto zigZagDelta : nonstd::span{values, count}) {
