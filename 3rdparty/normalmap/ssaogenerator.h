...
public:
    SsaoGenerator();
    osg::Image* calculateSsaomap(osg::Image* normalmap, osg::Image* depthmap, float radius,
                                 unsigned int kernelSamples, unsigned int noiseSize);

private:
    std::vector<osg::Vec3d> generateKernel(unsigned int size);
    std::vector<osg::Vec3d> generateNoise(unsigned int size);
