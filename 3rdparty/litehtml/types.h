...
namespace litehtml
{
	...
	struct variant : mpark::variant<Types...>
	{
		using base = variant<Types...>; // for derived class ctors
		using mpark::variant<Types...>::variant; // inherit ctors
		template<class T> bool is() const { return mpark::holds_alternative<T>(*this); }
		template<class T> const T& get() const { return mpark::get<T>(*this); }
		template<class T> T& get() { return mpark::get<T>(*this); }
	};
