...
void style::add_property(string_id name, const css_token_vector& value, const string& baseurl, bool important, document_container* container)
{
	...
	case _caption_side_:
        {
            int index = value_index(ident, m_valid_values[name]);
		    if (index >= 0) add_parsed_property(name, property_value(index, important));
        }
		break;
	...
	case _list_style_image_:
        {
            string url;
            if (parse_list_style_image(val, url))
		    {
...
bool parse_font_weight(const css_token& tok, css_length& weight)
{
    int idx = value_index(tok.ident(), font_weight_strings);
	if (idx >= 0)
	{
		weight.predef(idx);
		return true;
