...
#if _MSC_VER
#define THREAD_DEF __declspec(thread)
#else
#define THREAD_DEF __thread
#endif

THREAD_DEF int gk_cur_jbufs=-1;
THREAD_DEF jmp_buf gk_jbufs[MAX_JBUFS];
THREAD_DEF jmp_buf gk_jbuf;

typedef void (*gksighandler_t)(int);

/* These are the holders of the old singal handlers for the trapped signals */
static THREAD_DEF gksighandler_t old_SIGMEM_handler;  /* Custom signal */
static THREAD_DEF gksighandler_t old_SIGERR_handler;  /* Custom signal */
static THREAD_DEF gksighandler_t old_SIGMEM_handlers[MAX_JBUFS];  /* Custom signal */
static THREAD_DEF gksighandler_t old_SIGERR_handlers[MAX_JBUFS];  /* Custom signal */
