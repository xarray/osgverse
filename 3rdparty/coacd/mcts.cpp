...
namespace coacd
{
    ...
    // evaluate the quality until the mesh is all cut MAX_ROUND times
    double default_policy(Node *node, Params &params, vector<Plane> &current_path, bool& valid)
    {
        State *original_state = node->get_state();
        State current_state = *original_state;
        double current_state_reward; valid = true;
        ...
        while (current_state.is_terminal() == false)
        {
            ...
            if (!clipf)
                { valid = false; break; }//throw runtime_error("Wrong MCTS clip proposal!");
    ...
    Node *MonteCarloTreeSearch(Params &params, Node *node, vector<Plane> &best_path)
    {
        ...
        for (int i = 0; i < computation_budget; i++)
        {
            ...
            double reward = default_policy(expand_node, params, current_path, valid);
