...
btConvexHullInternal2::Int128 btConvexHullInternal2::Int128::operator*(int64_t b) const
{
...
btConvexHullInternal2::Int128 btConvexHullInternal2::Int128::mul(int64_t a, int64_t b)
{
    Int128 result;
...
btConvexHullInternal2::Int128 btConvexHullInternal2::Int128::mul(uint64_t a, uint64_t b)
{
    Int128 result;
...
int32_t btConvexHullInternal2::Rational64::compare(const Rational64& b) const
{
    if (sign != b.sign) {
        return sign - b.sign;
    }
...
int32_t btConvexHullInternal2::Rational128::compare(const Rational128& b) const
{
    if (sign != b.sign) {
        return sign - b.sign;
    }
...
int32_t btConvexHullInternal2::Rational128::compare(int64_t b) const
{
    if (isInt64) {
...
btConvexHullInternal2::Edge* btConvexHullInternal2::newEdgePair(Vertex* from, Vertex* to)
{
    btAssert(from && to);
...
bool btConvexHullInternal2::mergeProjection(IntermediateHull& h0, IntermediateHull& h1, Vertex*& c0, Vertex*& c1)
{
    Vertex* v0 = h0.maxYx;
    Vertex* v1 = h1.minYx;
...
void btConvexHullInternal2::computeInternal(int32_t start, int32_t end, IntermediateHull& result)
{
    int32_t n = end - start;
    switch (n) {
    case 0:
...
#ifdef DEBUG_CONVEX_HULL
void btConvexHullInternal2::IntermediateHull::print()
{
    printf("    Hull\n");
...
void btConvexHullInternal2::Vertex::printGraph()
{
    print();
    printf("\nEdges\n");
    Edge* e = edges;
...
btConvexHullInternal2::Orientation btConvexHullInternal2::getOrientation(const Edge* prev, const Edge* next, const Point32& s, const Point32& t)
{
...
btConvexHullInternal2::Edge* btConvexHullInternal2::findMaxAngle(bool ccw, const Vertex* start, const Point32& s, const Point64& rxs, const Point64& sxrxs, Rational64& minCot)
{
    Edge* minEdge = NULL;
...
void btConvexHullInternal2::findEdgeForCoplanarFaces(Vertex* c0, Vertex* c1, Edge*& e0, Edge*& e1, Vertex* stop0, Vertex* stop1)
{
    Edge* start0 = e0;
    Edge* start1 = e1;
...
void btConvexHullInternal2::merge(IntermediateHull& h0, IntermediateHull& h1)
{
    if (!h1.maxXy) {
        return;
    }
...
static bool pointCmp(const btConvexHullInternal2::Point32& p, const btConvexHullInternal2::Point32& q)
{
    return (p.y < q.y) || ((p.y == q.y) && ((p.x < q.x) || ((p.x == q.x) && (p.z < q.z))));
}

void btConvexHullInternal2::compute(vector<array<double, 3>> vertices)
{
...
btVector3 btConvexHullInternal2::toBtVector(const Point32& v)
{
    btVector3 p;
...
btVector3 btConvexHullInternal2::getBtNormal(Face* face)
{
    return toBtVector(face->dir0).cross(toBtVector(face->dir1)).normalized();
}

btVector3 btConvexHullInternal2::getCoordinates(const Vertex* v)
{
    btVector3 p;
    p[medAxis] = v->xvalue();
    p[maxAxis] = v->yvalue();
...
btScalar btConvexHullInternal2::shrink(btScalar amount, btScalar clampAmount)
{
    if (!vertexList) {
        return 0;
    }
...
bool btConvexHullInternal2::shiftFace(Face* face, btScalar amount, btAlignedObjectArray<Vertex*> stack)
{
...
static int32_t getVertexCopy(btConvexHullInternal2::Vertex* vertex, btAlignedObjectArray<btConvexHullInternal2::Vertex*>& vertices)
{
...
btScalar btConvexHullComputer2::compute(vector<array<double, 3>> points, btScalar shrink, btScalar shrinkClamp)
{
    ...
    btAlignedObjectArray<btConvexHullInternal2::Vertex*> oldVertices;
    getVertexCopy(hull.vertexList, oldVertices);
    int32_t copied = 0;
    while (copied < oldVertices.size()) {
        btConvexHullInternal2::Vertex* v = oldVertices[copied];
        vertices.push_back(hull.getCoordinates(v));
        btConvexHullInternal2::Edge* firstEdge = v->edges;
        if (firstEdge) {
            int32_t firstCopy = -1;
            int32_t prevCopy = -1;
            btConvexHullInternal2::Edge* e = firstEdge;
            do {
                if (e->copy < 0) {
    ...
    for (int32_t i = 0; i < copied; i++) {
        btConvexHullInternal2::Vertex* v = oldVertices[i];
        btConvexHullInternal2::Edge* firstEdge = v->edges;
        if (firstEdge) {
            btConvexHullInternal2::Edge* e = firstEdge;
            do {
                if (e->copy >= 0) {
                    ...
                    btConvexHullInternal2::Edge* f = e;
                    do {
#ifdef DEBUG_CONVEX_HULL
