...
namespace ifd {
	...
	bool FolderNode(const char* label, ImTextureID icon, bool& clicked)
	{
		...
		if (hovered || active)
			window->DrawList->AddRectFilled(g.CurrentWindow->Rect().Min, g.CurrentWindow->Rect().Max, ImGui::ColorConvertFloat4ToU32(ImGui::GetStyle().Colors[active ? ImGuiCol_HeaderActive : ImGuiCol_HeaderHovered]));
		...
		if (!(*state & 0b001)) {
			...
			for (size_t i = 0; i < btnList.size(); i++) {
				...
				if (ImGui::Button(btnList[i].c_str(), ImVec2(0, GUI_ELEMENT_SIZE))) {
					...
					path = ghc::filesystem::u8path(newPath);
					ret = true;
				}
				anyOtherHC |= ImGui::IsItemHovered() | ImGui::IsItemClicked();
				ImGui::SameLine();
		...
		// input box
			...
			if (ImGui::InputTextEx("##pathbox_input", "", pathBuffer, 1024, size_arg, ImGuiInputTextFlags_EnterReturnsTrue)) {
				...
				if (ghc::filesystem::exists(tempStr))
					path = ghc::filesystem::u8path(tempStr); 
				ret = true;
			}
	...
	bool FavoriteButton(const char* label, bool isFavorite)
	{
		...
		float size = g.CurrentWindow->Rect().Max.x - g.CurrentWindow->Rect().Min.x;
	...
	bool FileIcon(const char* label, bool isSelected, ImTextureID icon, ImVec2 size, bool hasPreview, int previewWidth, int previewHeight)
	{
		...
		if (hovered || active || isSelected)
			window->DrawList->AddRectFilled(g.CurrentWindow->Rect().Min, g.CurrentWindow->Rect().Max, ImGui::ColorConvertFloat4ToU32(ImGui::GetStyle().Colors[active ? ImGuiCol_HeaderActive : (isSelected ? ImGuiCol_Header : ImGuiCol_HeaderHovered)]));
	...
	bool FileDialog::Save(const std::string& key, const std::string& title, const std::string& filter, const std::string& startingDir)
	{
		...
		if (!startingDir.empty())
			m_setDirectory(ghc::filesystem::u8path(startingDir), false);
		else
	...
	void FileDialog::AddFavorite(const std::string& path)
	{
		...
		if (!ghc::filesystem::exists(ghc::filesystem::u8path(path)))
			return;
	...
	bool FileDialog::m_finalize(const std::string& filename)
	{
		...
		if (hasResult) {
			if (!m_isMultiselect || m_selections.size() <= 1) {
				ghc::filesystem::path path = ghc::filesystem::u8path(filename);
	...
	void FileDialog::m_setDirectory(const ghc::filesystem::path& p, bool addHistory)
	{
		...
		if (p.u8string().size() == 2 && p.u8string()[1] == ':')
			m_currentDirectory = ghc::filesystem::u8path(p.u8string() + "\\");
#endif
