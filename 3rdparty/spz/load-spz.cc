...
template<typename T> T clampBetween(const T& value, const T& low, const T& high)
{ return (value < low) ? low : (value > high) ? high : value; }
uint8_t toUint8(float x) { return static_cast<uint8_t>(clampBetween(std::round(x), 0.0f, 255.0f)); }
...
uint8_t quantizeSH(float x, int32_t bucketSize) {
  ...
  return static_cast<uint8_t>(clampBetween(q, 0, 255));
}
