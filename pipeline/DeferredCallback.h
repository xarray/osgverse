...
namespace osgVerse
{
    ...
    public:
        ...
        /** Specify on which camera the runners are executed and on which camera the depth
            buffer is blitted. Pipeline uses this to run deferred stages (bloom, luminance,
            ...) before the stages that consume their outputs; otherwise all the results
            generated by runners would be delayed by one frame. If not set, both jobs are
            done wherever this callback is attached, which keeps previous behaviour.
            Note: the callback needs to be registered (via setup()) on all these cameras */
        void setRunnerCamera(osg::Camera* cam) { _runnerCamera = cam; }
        void setBlitCamera(osg::Camera* cam) { _blitCamera = cam; }
        bool isForRunnerCamera(osg::Camera* cam) const
        { return !_runnerCamera.valid() || _runnerCamera == cam; }
