...
首先确认系统中已有`cmake`，`git`，以及必要的编译器程序。建议通过Visual Studio的Command Prompt进入终端界面。

进入脚本模式，为了避免在编译过程中等待用户输入，需要带一个CMD参数
```bash
Setup.bat <CMD>
```
...
脚本会自动下载OpenSceneGraph的代码仓库并放置于osgVerse根目录的同级目录。如果因为特殊原因导致其中文件损坏，可以直接删除该目录并在下次运行脚本时重新下载。
这里的<CMD>可以是: DEFAULT，CORE，GLES2，GLES3，WEBGL1，WEBGL2，ANDROID，对应之前所述的具体图形接口

## Compile under Linux

首先确认系统中已有`cmake`，`git`，以及必要的编译器程序，如`g++`或者`clang`。进入终端界面。
...
脚本模式可以带第2个参数，例如
```bash
Setup.sh <CMD> <path_to_gles_libs>
# 此处假定path_to_gles_libs是libGLESv2.so和libEGL.so库所在目录
```

脚本会自动下载OpenSceneGraph的代码仓库并放置于osgVerse根目录的同级目录。如果因为特殊原因导致其中文件损坏，可以直接删除该目录并在下次运行脚本时重新下载。
对应之前所述的具体图形接口，这里的<CMD>可以是: DEFAULT，CORE，GLES2，GLES3，WEBGL1，WEBGL2，ANDROID
...
通过鼠标左键拖曳，缓缓旋转模型，观察其阴影、光照效果以及背景天空盒是否均存在，且渲染结果是否流畅自然。
