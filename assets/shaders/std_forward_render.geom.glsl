...
VERSE_VS_IN vec4 texCoord0_gs[], texCoord1_gs[], color_gs[], eyeVertex_gs[];
...
uniform mat4 viewMatrices[2], projMatrices[2];
...
void main()
{
    ...
    for (int k = 0; k < numViews; ++k)
    {
        ...
        mat4 MVP = TSP * projMatrices[k] * viewMatrices[k];
        for (int i = 0; i < 3; ++i)
        {
            vec4 tmp = MVP * VERSE_GS_POS(i);
            ...
            eyeVertex = viewMatrices[k] * eyeVertex_gs[i];
            gl_Position = tmp;
