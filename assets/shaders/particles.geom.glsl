...
void main()
{
    ...
    for (int i = 0; i < 6; ++i)
    {
        ...
        {
            vec4 viewPos = vec4(e[id], 0.0, 0.0) + gl_PositionIn[0];
        ...
        {
            vec4 viewPos = gl_PositionIn[0] + vec4(e[id] * texCoord_gs[0].x, 0.0, 0.0);
            gl_Position = VERSE_MATRIX_P * viewPos;
        }
        else if (DataRange.z > 0.5)  // billboard (with-scale)
        {
            vec4 viewPos = VERSE_MATRIX_MVP * (vec4(e[id], 0.0, 0.0) + gl_PositionIn[0]);
            gl_Position = viewPos + vec4(e[id] * texCoord_gs[0].x, 0.0, 0.0);
        }
        else  // etc.
            gl_Position = VERSE_MATRIX_MVP * (vec4(e[id], 0.0, 0.0) + gl_PositionIn[0]);
        EmitVertex();
